...
import { translateResultFailEmbed, translateResultSuccessEmbed } from "../embeds";
...
export const papagoTranslate: Command = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption((option): SlashCommandStringOption => {
            return option.setName('from')
                ...
                .setRequired(true)
                .addChoices(...supportedLanguages.reduce((acc: SlashCommandChoice[], cur: PapagoLanguageSet, idx, src) => {
                    acc.push({
                        name: cur.name,
                        value: cur.key
                    })
        ...
        .addStringOption((option): SlashCommandStringOption => {
            return option.setName('to')
                ...
                .setRequired(true)
                .addChoices(...supportedLanguages.reduce((acc: SlashCommandChoice[], cur, idx, src) => {
                    acc.push({
                        name: cur.name,
                        value: cur.key
                    })
    ...
    run: async (client: DiscordCustomClient, interaction: CommandInteraction) => {
        ...
        logger.info(`Translation Request(${requestUserName}) - ${from} → ${to} - Content : ${content}`)
        const result = await papagoRequest(from as string, to as string, content)
        try {
            if (result.type === 'success') {
                await interaction.reply({
                    embeds: [
                        await translateResultSuccessEmbed(fromName as string, toName as string, content, result.result),
                    ],
                    ephemeral
                })
            } else {
                await interaction.editReply({
                    embeds: [
                        await translateResultFailEmbed(result.errormessage)
                    ],
                })
            }
        } catch (err) {
            ...
            await interaction.reply({
                embeds: [
                    await translateResultFailEmbed("😓 Content should be less than 1024 characters!")
                ],
                ephemeral
            })
        }
